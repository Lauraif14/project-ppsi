...
import RiwayatPiketUser from "./RiwayatPiketUser";
...
const InventarisChecklist = ({ sesiAbsen, setSesiAbsen, onChecklistSubmit }) => {
  ...
  const handleSubmitChecklist = async () => {
    if (!allChecked) {
      alert(
        `Mohon lengkapi semua item checklist (${checkedItems}/${totalItems})`
      );
      return;
    }
    ...
    try {
      const response = await api.post(
        ...
        { absensiId: sesiAbsen.id, checklist },
        {
    ...
    <div className="p-6 border-2 border-black rounded-xl bg-white shadow-lg">
      ...
      <div className="space-y-3 max-h-80 overflow-y-auto pr-2">
        {checklist.map((item, index) => (
          <div
            key={index}
            className="p-4 border-2 border-gray-200 rounded-lg bg-gray-50 hover:border-pink-300 transition-colors"
          >
...
const DashboardUser = () => {
  ...
  const { isDurasiCukup, durasiMenit, sisaMenit } = useMemo(() => {
    if (!sesiAbsen?.waktu_masuk)
      return { isDurasiCukup: false, durasiMenit: 0, sisaMenit: 0 };
    ...
    return { isDurasiCukup: durasiMenit >= 120, durasiMenit, sisaMenit };
  }, [sesiAbsen]);
      ...
      <div className="max-w-7xl mx-auto p-6 lg:p-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-8 space-y-8">
            {!sesiAbsen ? (
              <div className="p-10 border-2 border-black rounded-xl bg-white shadow-lg text-center">
                ...
                <h2 className="text-3xl font-bold mb-3">Mulai Sesi Piket</h2>
                <p className="text-gray-600 mb-8">
                  Anda belum memulai sesi piket hari ini.
                </p>
                <button
                  onClick={() => handleOpenAbsen("masuk")}
                  className="px-10 py-4 bg-green-500 text-white font-bold text-lg rounded-xl border-2 border-black hover:bg-green-600 transition-all flex items-center justify-center gap-3 mx-auto"
                >
                  <Camera size={24} />
                  Ambil Absen Masuk
                </button>
                ...
                <div className="p-6 border-2 border-black rounded-xl bg-pink-50 shadow-lg">
                  <div className="flex items-start justify-between">
                    <div className="flex gap-4">
                      <div className="w-16 h-16 bg-pink-500 rounded-xl flex items-center justify-center border-2 border-black">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-700">
                          ...
                          <span className="font-semibold">
                            {new Date(sesiAbsen.waktu_masuk).toLocaleTimeString(
                              "id-ID"
                            )}
                          </span>
                        </p>
                        <p className="text-sm text-gray-700">
                          Durasi:{" "}
                          <span className="font-semibold">
                            {Math.floor(durasiMenit / 60)} jam{" "}
                            {Math.floor(durasiMenit % 60)} menit
                          </span>
                        </p>
                      </div>
                    </div>
            ...
            {/* Form Ajukan Izin */}
            <div className="border-2 border-yellow-400 bg-yellow-50 p-6 rounded-xl shadow-lg">
              ...
              <form
                onSubmit={async (e) => {
                  ...
                  if (!tanggal || !alasan)
                    return alert("Tanggal dan alasan wajib diisi!");
                  try {
                    await api.post(
                      "/riwayat/izin",
                      { tanggal, alasan },
              ...
              >
                <div>
                  <label className="block font-semibold mb-1">Tanggal Izin</label>
                  <input
                    type="date"
                    name="tanggal"
                    className="w-full border-2 border-black rounded-lg p-2 focus:outline-none focus:border-yellow-500"
                  />
                </div>
                <div>
                  <label className="block font-semibold mb-1">Alasan</label>
                  <textarea
                    name="alasan"
                    className="w-full border-2 border-black rounded-lg p-2 h-20 focus:outline-none focus:border-yellow-500"
                  ></textarea>
                </div>
                <button
                  type="submit"
                  className="px-5 py-3 bg-yellow-500 text-white font-bold rounded-lg border-2 border-black hover:bg-yellow-600 transition-all"
                >
                  Kirim Izin
                </button>
              </form>
            ...
            <div className="p-6 border-2 border-black rounded-xl bg-white shadow-lg">
              <h3 className="text-xl font-bold mb-4 flex items-center gap-2 text-gray-800">
                <span className="text-pink-500">📘</span> SOP & Panduan Piket
              </h3>
              <SOPList />
            </div>
          </div>
          ...
          <div className="lg:col-span-4 space-y-8">
            ...
            {/* ✅ Riwayat dipindah ke bawah Jadwal */}
            <RiwayatPiketUser />

            {sesiAbsen && (
              <div className="p-6 border-2 border-black rounded-xl bg-white shadow-lg">
                <h3 className="font-bold text-xl mb-5 text-gray-800">
                  Persyaratan Absen Keluar
                </h3>
                ...
                <button
                  ...
                  className="w-full py-4 bg-red-500 text-white font-bold text-lg rounded-lg border-2 border-black hover:bg-red-600 disabled:bg-gray-400 transition-all flex items-center justify-center gap-2"
                >
                  <Camera size={24} />
                  {canAbsenKeluar
                    ? "Ambil Absen Keluar"
