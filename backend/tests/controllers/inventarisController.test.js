...
describe('InventarisController (OOP)', () => {
    ...
    describe('updateInventaris', () => {
        const mockExistingItem = { id: 1, nama_barang: 'Keyboard', jumlah: 5, status: 'Tersedia' };
        const mockUpdatedItem = { id: 1, nama_barang: 'Keyboard Updated', jumlah: 10, status: 'Rusak' };

        beforeEach(() => {
            req.params = { id: '1' };
            req.body = { nama_barang: 'Keyboard Updated', jumlah: '10', status: 'Rusak' };
            InventarisModel.findInventarisById.mockResolvedValueOnce(mockExistingItem).mockResolvedValueOnce(mockUpdatedItem);
            InventarisModel.updateInventaris.mockResolvedValue(true);
        });

        test('should update item successfully with valid data', async () => {
            await bind(inventarisControllerInstance.updateInventaris)(req, res);

            expect(InventarisModel.updateInventaris).toHaveBeenCalledWith(
                '1',
                expect.objectContaining({ nama_barang: 'Keyboard Updated', jumlah: 10, status: 'Rusak' })
            );
            expect(res.json).toHaveBeenCalledWith(
                expect.objectContaining({ success: true, message: 'Inventaris berhasil diupdate', data: mockUpdatedItem })
            );
        });
        ...
        test('should return 404 if item not found', async () => {
            InventarisModel.findInventarisById.mockReset().mockResolvedValue(null);
        ...
        test('should return 400 if jumlah is missing', async () => {
            req.body = { nama_barang: 'Test' };
            await bind(inventarisControllerInstance.updateInventaris)(req, res);
            expect(res.status).toHaveBeenCalledWith(400);
        });

        test('should return 400 if jumlah is not a number', async () => {
            req.body = { nama_barang: 'Test', jumlah: 'invalid' };
            await bind(inventarisControllerInstance.updateInventaris)(req, res);
            expect(res.status).toHaveBeenCalledWith(400);
        ...
        test('should return 400 if status is invalid', async () => {
            req.body = { nama_barang: 'Test', jumlah: '5', status: 'InvalidStatus' };
            await bind(inventarisControllerInstance.updateInventaris)(req, res);
            expect(res.status).toHaveBeenCalledWith(400);
            expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ message: expect.stringContaining('Status tidak valid') }));
        });
        ...
        test('should return 500 on database error during update', async () => {
            ...
            expect(res.status).toHaveBeenCalledWith(500);
            expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ success: false, message: 'Error updating inventaris' }));
        });
    });
